 ...
 * @author freckie
 */
public class TiberoNetConnectionDescriptorTokenizer {
    ...
    public TiberoNetConnectionDescriptorTokenizer(String connectionString) {
        ...
        for (; position < length; position++) {
            ...
            switch (ch) {
                ...
                case TOKEN_SQUOTE:
                    ...
                    throw new TiberoConnectionStringException("unsupported token:" + ch);
                default:
    ...
    public void checkStartToken() {
        ...
        if (token == null) {
            throw new TiberoConnectionStringException("parse error. token is null");
        }
        // We can check by == because the token object is singleton.
        if (!(token == TOKEN_KEY_START_OBJECT)) {
            throw new TiberoConnectionStringException("syntax error. Expected token='(' :" + token.getToken());
        }
    }
    ...
    public void checkEqualToken() {
        Token token = this.nextToken();
        if (token == null) {
            throw new TiberoConnectionStringException("parse error. token is null. Expected token='='");
        }
        // We can check by == because the token object is singleton.
    ...
    public Token getLiteralToken() {
        Token token = this.nextToken();
        if (token == null) {
            throw new TiberoConnectionStringException("parse error. token is null. Expected token='LITERAL'");
        }
        // We can check by == because the token object is singleton.
        if (!(token.getType() == TYPE_LITERAL)) {
            throw new TiberoConnectionStringException("Syntax error. Expected token='LITERAL'' :" + token.getToken());
        }
        return token;
    }
    ...
    public Token getLiteralToken(String expectedValue) {
        ...
        if (!expectedValue.equals(token.getToken())) {
            throw new TiberoConnectionStringException("Syntax error. Expected token=" + expectedValue + "' :" + token.getToken());
        }
        return token;
    }
}
